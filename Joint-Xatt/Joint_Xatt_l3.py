...
class ECPEC(nn.Module):
    ...
    def __init__(self, input_dim, n_class, dropout, acti):
        ...
        self.Wsa = nn.Linear(4*input_dim, self.n_class)
        self.Wcd = nn.Linear(4*input_dim, self.n_class)
        ...
        self.wgru = nn.LSTM(input_size=input_dim, hidden_size=int(input_dim/2), num_layers=2, batch_first=True, bidirectional=True)

        self.saatt = nn.Parameter(kaiming_uniform_(torch.zeros((2*input_dim, 2*input_dim))))
        self.cdatt = nn.Parameter(kaiming_uniform_(torch.zeros((2*input_dim, 2*input_dim))))
    ...
    def forward(self, x, sen_lengths, mask):
        ...
        backward, (hnc,cnc) = self.cgru(conb, (hc,cc))
        backward = self._reverse_seq(backward, mask)
        ConC = torch.cat((forward, backward), dim=2)


        # information share for sentiment analysis
        att = ConE.transpose(0, 1).matmul(self.saatt).matmul(ConC.permute(1,2,0))
        att_score1 = F.softmax(att, dim=2)
        attf1 = att_score1.matmul(ConC.transpose(0, 1)).transpose(0, 1)
        emoe = torch.cat((ConE, attf1), dim=2)


        # information share for cause utterance detection
        att = ConC.transpose(0, 1).matmul(self.cdatt).matmul(ConE.permute(1,2,0))
        att_score2 = F.softmax(att, dim=2)
        attf2 = att_score2.matmul(ConE.transpose(0, 1)).transpose(0, 1)
        emoc = torch.cat((ConC, attf2), dim=2)
        ...
        # indicator = torch.cat((hne, cne), dim=-1).squeeze()
        # lambda_ = self.sig(self.indicator(indicator))


        return results_sa, results_cd  #, lambda_.item()
...
if __name__ == '__main__':
    ...
    for e in range(n_epochs):
        ...
        train_loss, train_acc, train_acc_c, _,_,_,_,_, train_fscore, train_fscore_c, sa_pred, cd_pred = train_or_eval_model(model, loss_function, loss_function_cd,
                                                train_loader, e, prop, embedding, optimizer, True)
        valid_loss, valid_acc, valid_acc_c,_,_,_,_,_, val_fscore, val_fscore_c, _, _ = train_or_eval_model(model, loss_function, loss_function_cd, valid_loader, e, prop, embedding)
        test_loss, test_acc, test_acc_c, test_labele, test_prede, test_labelc, test_predc, test_mask, test_fscore, test_fscore_c, saTask, cdTask = train_or_eval_model(model, loss_function, loss_function_cd, test_loader, e, prop, embedding)
    ...
    print('Loss {} accuracy {}'.format(best_loss,
                                     round(accuracy_score(best_labele, best_prede, sample_weight=best_mask)*100,2)))
    ...
    path = './pkl_xatt_full/ECPEC_phase_two_xatt_' + str(args.dropout) + '_' + str(args.proportion) + '_' + str(args.activation) + '_att_2' + '.pkl'
    f = open(path, 'wb')
    data = [videoIDs, videoSpeakers, videoLabels, causeLabels, causeLabels2, causeLabels3, saTrain, cdTrain, best_sa, best_cd, videoText, videoAudio, videoVisual, videoSentence, trainVid, testVid]
    pickle.dump(data, f)
    f.close()
