...
from dataloader_v5 import IEMOCAPDataset
...
class ECPEC(nn.Module):
    ...
    def __init__(self, input_dim, n_class, dropout):
        ...
        self.lstm = nn.LSTM(input_size=input_dim,hidden_size=input_dim,num_layers=1,
                        bias=True,batch_first=False,dropout=0.2,bidirectional=False)
    ...
    def forward(self, text_emo, text_cau, chunksize=8, t_ratio=1, label_ck=None,
        ...
        c0 = torch.zeros((1, text_emo.size(1), text_emo.size(2))).cuda()
        ...
        for j, emo in enumerate(text_emo):
            ...
            # employ LSTM to generate chunk embedding chEmb and chunk-emotion embedding
            for i, ch in enumerate(chunks):
                # h0 = torch.zeros((1, text_emo.size(1), text_emo.size(2))).cuda()
                chEmb, (h0,c0) = self.lstm(ch, (h0, c0))
...
def train_model(model, loss_function, loss_function_c, dataloader, epoch, t_ratio=0.2, smth=0, smth3=0.0, optimizer=None, train=True):
    losses = []
    ...
    for data in dataloader:
        ...
        label_ = label.view(-1)
        label_sm = smooth_one_hot(label_, 2, smoothing=0.2)
...
def eval_model(model, loss_function, loss_function_c, dataloader, epoch, t_ratio=0.0, smth=0, smth3=0.00, optimizer=None,
                        train=False):
    losses = []
    # phase2 and phase3 tasks
    predec = []
    ...
    for data in dataloader:
        ...
        if text_emo.size(0) ==0 or text_cau.size(0) ==0:
            ...
            if not skip:
                loss_3 = LSloss(log_phase3, label_phase3) if smth3 > 0 else loss_function_c(log_phase3, label_phase3)
                #loss_3 = loss_function_c(log_phase3, label_phase3)
                #n2 = log_phase3.size(0)
                loss = loss_2 + loss_3
...
if __name__ == '__main__':
    ...
    train_loader, valid_loader, test_loader = \
        get_IEMOCAP_loaders(r'../ECPEC_phase_two_GCN_best.pkl',
                            valid=0.0,
                            batch_size=batch_size,
                            num_workers=2)
