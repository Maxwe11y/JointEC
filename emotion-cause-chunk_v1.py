...
class ECPEC(nn.Module):
    ...
    def forward(self, text_emo, text_cau, chunksize=8, t_ratio=1, label_ck=None,
        ...
        chunks = torch.split(text_cau, chunksize, 0)  # [num_chunk, chunksize, dim] --> tuple
...
def train_or_eval_model(model, loss_function, dataloader, epoch, t_ratio=0.0, optimizer=None, train=False):
    losses = []
    ...
    for data in dataloader:
        ...
        if not skip:
            loss_3 = loss_function(log_phase3, label_phase3.view(-1))
            loss = loss_2 + loss_3
        else:
            loss = loss_2
        ...
        # ecTask[data[-1][0]] = list(pred_e.data.cpu().numpy())
...
if __name__ == '__main__':
    ...
    for e in range(n_epochs):
        ...
        train_loss, train_accuracy_ec, _, _, train_fscore_ec, train_accuracy_p3, _, _, train_fscore_p3 = train_or_eval_model(model, loss_function, train_loader, e, optimizer=optimizer, train=True)
        valid_loss, valid_accuracy_ec, _, _, valid_fscore_ec, valid_accuracy_p3, _, _, valid_fscore_p3 = train_or_eval_model(model, loss_function, valid_loader, e)
        test_loss, test_accuracy_ec, test_labelec, test_predec, test_fscore_ec, test_accuracy_p3, \
        test_labelp3, test_pred3, test_fscore_p3 = train_or_eval_model(model, loss_function, test_loader, e, t_ratio=0.0)
